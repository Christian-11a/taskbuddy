...
Enable RLS on **every** table. **Since migration 0026, signed-in users read and never write.** Every
write goes through the API, which uses the service-role key and so bypasses RLS; no frontend talks
to PostgREST (§20). The write policies 0003 originally granted — own profile, own provider row, own
jobs, application status, reviews, messages, checklist items, notification read-state — let a
user skip the checks the API makes, most seriously setting their own `is_verified` (§17) or
accepting an application without escrow ever holding the money (§28.3). 0026 drops them.

| Table | Policy intent |
|---|---|
| `profiles` | Users read own row; authenticated users read basic profile info for marketplace display. Writes: API only. |
| `service_categories`, `urgency_settings` | Read for all authenticated users; write via service role only. |
| `provider_profiles` | Authenticated users can read (needed for job browsing/display). Writes: API/triggers only — including `is_verified` and the `cached_*` columns. |
| `jobs` | Client reads own jobs. Providers read jobs with status `open`/`recommending`, plus jobs they're assigned to. Writes: API only. |
| `job_applications` | Provider reads own applications; client reads applications for own jobs. Writes: API only — an accept is an escrow hold first. |
...
| `reviews` | Read for all authenticated users. Inserted by the API for completed jobs (one per job, enforced by unique constraint). |
| `notifications` | Recipient reads own rows; inserts via service role/triggers, mark-read via the API. |
...
**`is_verified` is a gate, not a badge.** It was first specified as a badge, on the grounds that
gating would lock out every provider who signed up before verification existed. The code gated it
anyway (`381836d`), the two disagreed for a while, and the decision was then made deliberately in
the code's favour: this is a home-services marketplace, and a hire hands a stranger a client's
address. An unverified provider can see the feed but cannot act on it.

| Where | Refusal | Why there |
|---|---|---|
| `ApplicationsService.apply` | `403 { code: 'verification_required' }` | The first point at which a provider asks for work. The `code` lets the app offer "Verify your identity" instead of a dead end |
| `ApplicationsService.assertHireable` (accept, and the card-at-hire checkout) | `409 { code: 'provider_not_verified' }` | Checked again before any money is held: an application can outlive the verification it was filed under |

The gate is only as strong as the flag's write path. Migration 0026 drops the RLS policies that let
a signed-in user update their own `provider_profiles` row (and several others — §11); since then
`is_verified` is set only by an admin approval or the Stripe Identity webhook.
...
**400 `Insufficient wallet balance`** when the client's *available* balance (§27.2) is below the job
budget — the job stays `open` and nobody is hired. Clients add funds through a Stripe top-up
(`POST /payments/checkout-session`, §21), which is what mobile's Add Money button does; the wallet
is credited by the signed webhook, never by the request. `POST /wallet/transactions` refuses
`direction: 'credit'` from every caller.

The ledger is still the only account of record: Stripe decides whether money arrived, and
`wallet_transactions` records it.
