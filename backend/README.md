...
2. Apply **every** migration in [`supabase/migrations/`](./supabase/migrations) **in order**
   ...
   | `0032_matching_eligibility.sql` | replaces `fn_job_provider_features`' WHERE clause: providers must be verified, have a bio, and be within their own `service_radius_km` (same category or not). Without it, one signup-created provider with a NULL bio makes ml-service reject the whole scoring batch. Same signature and features. Re-runnable. See `BACKEND_SCHEMA.md` §32. |
...
| Method & path | Description |
|---|---|
| `PATCH /profiles/me` 🔒 | update `full_name, phone, avatar_url, address, city`. A changed `address`/`city` (or a saved address with no coordinates yet) is geocoded server-side and stores `latitude`/`longitude`; an unverifiable address rejects the save with `400`, and `503` if geocoding is unavailable. Coordinates are never accepted from the client (`BACKEND_SCHEMA.md` §32). `avatar_url` takes either an `avatars` Storage path (converted to a public URL) or an `https://` URL; `""` clears it |
...
- Jobs left in `recommending` with no run (ml-service failed, or nobody was
  eligible yet) are retried on later ticks until scored or expired.
