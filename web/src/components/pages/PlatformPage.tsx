...
const inputStyle = { background: "var(--input-bg)", border: "1px solid var(--border-md)", borderRadius: 9, padding: "9px 11px", fontSize: 12, color: "var(--text-white)", fontFamily: "inherit" };
...
export function PlatformPage() {
  ...
  return <div>
    <div className="flex items-start justify-between flex-wrap gap-3 mb-4"><div><div className="text-white font-bold" style={{ fontSize: 22, letterSpacing: "-0.025em" }}>Platform</div><div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 5, lineHeight: 1.45 }}>Manage the marketplace rules, catalogue, administrators, and announcements.</div></div><button onClick={() => { setRefreshing(true); void load(true); }} disabled={refreshing} className="flex items-center gap-1.5 font-semibold disabled:opacity-40" style={{ background: "var(--chip-bg)", border: "1px solid var(--border-md)", borderRadius: 10, padding: "8px 12px", fontSize: 11.4, color: "var(--text-light)", cursor: "pointer", fontFamily: "inherit" }}><RefreshCw size={12} /> Refresh</button></div>
    {error && <div role="alert" className="flex items-center gap-2 rounded-xl mb-4" style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.25)", padding: "10px 14px", fontSize: 11.5, color: "var(--danger-text)" }}><XCircle size={14} />{error}</div>}
    <div className="inline-flex flex-wrap mb-4" style={{ background: "var(--chip-bg)", padding: 3, borderRadius: 10, gap: 2 }}>{tabs.map((item) => <button key={item.id} onClick={() => setTab(item.id)} className="flex items-center gap-1.5 rounded-lg font-medium" style={{ padding: "8px 12px", fontSize: 11.2, background: tab === item.id ? "var(--indigo-dark)" : "transparent", color: tab === item.id ? "var(--indigo-light)" : "var(--text-muted)", border: "none", cursor: "pointer", fontFamily: "inherit" }}>{item.icon}{item.label}</button>)}</div>
    {loading ? <div className="flex items-center justify-center" style={{ height: 280, color: "var(--text-muted)", fontSize: 12 }}>Loading platform controls…</div> : <>
      {tab === "commission" && <Section title="Platform commission" description="Set the fraction retained from future escrow releases. Existing settled jobs keep their original figures." icon={<Percent size={15} />}><form onSubmit={saveCommission} className="flex items-end gap-3 flex-wrap"><label className="block" style={{ fontSize: 11, color: "var(--text-muted)" }}>Commission rate (%)<input type="number" min="0" max="50" step="0.01" value={rate} onChange={(e) => setRate(e.target.value)} style={{ ...inputStyle, display: "block", width: 180, marginTop: 6 }} /></label><button disabled={busy} className="font-semibold disabled:opacity-40" style={{ background: "linear-gradient(172deg, #22c3d6, #38bdf8)", border: "none", borderRadius: 9, padding: "9px 15px", color: "#fff", fontSize: 11.4, cursor: "pointer", fontFamily: "inherit" }}>{busy ? "Saving…" : "Save rate"}</button></form>{commission && <div className="flex items-center gap-2 mt-4" style={{ fontSize: 10.5, color: "var(--text-muted)" }}><CheckCircle2 size={13} style={{ color: "var(--success-text)" }} /> Current rate: <strong style={{ color: "var(--text-white)" }}>{(commission.rate * 100).toFixed(2)}%</strong> · Last updated {formatDate(commission.updatedAt)}</div>}<div className="rounded-lg mt-5" style={{ background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.18)", padding: "10px 12px", color: "var(--warning-text)", fontSize: 10.5, lineHeight: 1.45 }}>A rate of 15% is entered as <strong>15</strong> here. The API receives the safe fraction <strong>0.15</strong> and caps it at 50%.</div></Section>}
      {tab === "categories" && <Section title="Service categories" description="Rename categories or deactivate them from new job forms. Deactivation preserves historical jobs; deletion is intentionally unavailable." icon={<FolderCog size={15} />}><form onSubmit={addCategory} className="flex gap-2 mb-4 flex-wrap"><input aria-label="New category name" value={categoryName} onChange={(e) => setCategoryName(e.target.value)} maxLength={60} placeholder="New category name" style={{ ...inputStyle, flex: "1 1 240px" }} /><button disabled={busy || !categoryName.trim()} className="flex items-center gap-1.5 font-semibold disabled:opacity-40" style={{ background: "var(--indigo-dark)", border: "1px solid rgba(34,195,214,0.3)", borderRadius: 9, padding: "8px 13px", color: "var(--indigo-light)", fontSize: 11.4, cursor: "pointer", fontFamily: "inherit" }}><Plus size={13} /> Add category</button></form><div className="overflow-x-auto"><table className="data-table"><thead><tr><th>Name</th><th>Status</th><th style={{ width: 190 }}>Actions</th></tr></thead><tbody>{categories.map((item) => <tr key={item.id}><td>{editingCategory === item.id ? <input autoFocus value={editCategoryName} onChange={(e) => setEditCategoryName(e.target.value)} maxLength={60} style={{ ...inputStyle, width: "100%" }} /> : <span className="text-white font-medium" style={{ fontSize: 11.5 }}>{item.name}</span>}</td><td><span className={`badge ${item.isActive ? "badge-active" : "badge-cancelled"}`}>{item.isActive ? "Active" : "Inactive"}</span></td><td>{editingCategory === item.id ? <div className="flex gap-1.5"><button onClick={() => void saveCategory(item.id)} className="font-semibold" style={{ ...inputStyle, padding: "6px 9px", color: "var(--success-text)", cursor: "pointer" }}>Save</button><button onClick={() => setEditingCategory(null)} className="font-semibold" style={{ ...inputStyle, padding: "6px 9px", color: "var(--text-muted)", cursor: "pointer" }}>Cancel</button></div> : <div className="flex gap-1.5"><button onClick={() => { setEditingCategory(item.id); setEditCategoryName(item.name); }} className="font-semibold" style={{ ...inputStyle, padding: "6px 9px", color: "var(--text-light)", cursor: "pointer" }}>Rename</button><button onClick={() => void toggleCategory(item)} disabled={busy} className="font-semibold disabled:opacity-40" style={{ ...inputStyle, padding: "6px 9px", color: item.isActive ? "var(--warning-text)" : "var(--success-text)", cursor: "pointer" }}>{item.isActive ? "Deactivate" : "Activate"}</button></div>}</td></tr>)}</tbody></table></div>{categories.length === 0 && <div className="text-center py-8" style={{ color: "var(--text-muted)", fontSize: 12 }}>No categories found.</div>}</Section>}
      {tab === "admins" && <Section title="Admin accounts" description="Invite another administrator without sharing passwords. New admins receive a reset email to set their own credential." icon={<ShieldCheck size={15} />}><form onSubmit={addAdmin} className="grid grid-cols-1 md:grid-cols-[1fr_1fr_auto] gap-2 mb-5"><input aria-label="Admin full name" value={adminName} onChange={(e) => setAdminName(e.target.value)} maxLength={120} placeholder="Full name" style={inputStyle} /><input aria-label="Admin email" type="email" value={adminEmail} onChange={(e) => setAdminEmail(e.target.value)} placeholder="name@example.com" style={inputStyle} /><button disabled={busy || !adminName.trim() || !adminEmail.trim()} className="flex items-center justify-center gap-1.5 font-semibold disabled:opacity-40" style={{ background: "var(--indigo-dark)", border: "1px solid rgba(34,195,214,0.3)", borderRadius: 9, padding: "8px 13px", color: "var(--indigo-light)", fontSize: 11.4, cursor: "pointer", fontFamily: "inherit" }}><UserRoundPlus size={13} /> Add admin</button></form><div className="overflow-x-auto"><table className="data-table"><thead><tr><th>Administrator</th><th>Created</th><th>Status</th><th style={{ width: 120 }}>Actions</th></tr></thead><tbody>{admins.map((item) => <tr key={item.id}><td><div className="flex items-center gap-2.5"><div className="flex items-center justify-center rounded-lg" style={{ width: 30, height: 30, background: "var(--indigo-dark)", color: "var(--indigo-light)" }}><UserRound size={14} /></div><div><div className="text-white font-medium" style={{ fontSize: 11.5 }}>{item.name}</div><div style={{ fontSize: 9.8, color: "var(--text-muted)" }}>{item.email}</div></div></div></td><td style={{ color: "var(--text-light)", fontSize: 11.2 }}>{formatDate(item.createdAt)}</td><td><span className={`badge ${item.deactivatedAt || item.deletedAt ? "badge-cancelled" : "badge-active"}`}>{item.deactivatedAt || item.deletedAt ? "Revoked" : "Active"}</span></td><td>{!item.deactivatedAt && !item.deletedAt && <button onClick={() => setRevoking(item)} className="font-semibold" style={{ ...inputStyle, padding: "6px 9px", color: "var(--danger-text)", cursor: "pointer" }}>Revoke</button>}</td></tr>)}</tbody></table></div></Section>}
      {tab === "broadcast" && <Section title="Broadcast announcement" description="Send an in-app notification to active, non-admin users. Suspended and deleted accounts are excluded automatically." icon={<BellRing size={15} />}><form onSubmit={sendBroadcast} className="max-w-2xl"><label className="block mb-3" style={{ fontSize: 11, color: "var(--text-muted)" }}>Audience<select value={audience} onChange={(e) => setAudience(e.target.value as typeof audience)} style={{ ...inputStyle, display: "block", width: "100%", marginTop: 6 }}><option value="all">All users</option><option value="clients">Homeowners only</option><option value="providers">Providers only</option></select></label><label className="block mb-3" style={{ fontSize: 11, color: "var(--text-muted)" }}>Title<input value={title} onChange={(e) => setTitle(e.target.value)} maxLength={120} placeholder="Short announcement title" style={{ ...inputStyle, display: "block", width: "100%", marginTop: 6 }} /><span style={{ display: "block", textAlign: "right", marginTop: 3, fontSize: 9 }}>{title.length}/120</span></label><label className="block mb-4" style={{ fontSize: 11, color: "var(--text-muted)" }}>Message<textarea value={body} onChange={(e) => setBody(e.target.value)} maxLength={500} rows={6} placeholder="What should users know?" style={{ ...inputStyle, display: "block", width: "100%", marginTop: 6, resize: "vertical" }} /><span style={{ display: "block", textAlign: "right", marginTop: 3, fontSize: 9 }}>{body.length}/500</span></label><button disabled={busy || !title.trim() || !body.trim()} className="flex items-center gap-1.5 font-semibold disabled:opacity-40" style={{ background: "linear-gradient(172deg, #22c3d6, #38bdf8)", border: "none", borderRadius: 9, padding: "9px 15px", color: "#fff", fontSize: 11.4, cursor: "pointer", fontFamily: "inherit" }}><BellRing size={13} /> {busy ? "Sending…" : "Send announcement"}</button></form></Section>}
    </>}
