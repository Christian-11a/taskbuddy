  ...
  return (
    ...
    >
      <div className="flex items-center" style={{ gap: 14, minWidth: 0 }}>
        ...
        <span className="hidden md:inline font-bold whitespace-nowrap" style={{ fontSize: "var(--fs-md)" }}>TaskBuddy Admin</span>
        ...
        <input
          ...
          style={{ height: 38, background: "var(--input-bg)", border: "1px solid var(--border-md)", borderRadius: "var(--r-md)", padding: "0 13px 0 36px", fontSize: "var(--fs-sm)", color: "var(--text-white)" }}
        />
        {searchOpen && searchQuery.trim().length >= 2 && (
          ...
          >
            {searchResults.length === 0 ? (
              <div className="text-center py-6" style={{ fontSize: "var(--fs-xs)", color: "var(--text-muted)" }}>
              ...
              searchResults.map((r) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-white font-medium truncate" style={{ fontSize: "var(--fs-xs)" }}>{r.label}</div>
                    <div className="truncate" style={{ fontSize: "var(--fs-3xs)", color: "var(--text-muted)", marginTop: 1 }}>{r.sub}</div>
                  </div>
                </Link>
              ))
            )}
      ...
      <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
        <span className="hidden lg:block" style={{ fontSize: "var(--fs-xs)", color: "var(--text-muted)" }}>{today}</span>
        ...
        <div className="relative" ref={notifRef}>
          <button
            ...
            style={{ width: 36, height: 36, background: "var(--input-bg)", borderRadius: "var(--r-md)", border: "none", cursor: "pointer", color: "var(--text-muted)" }}
          >
            <Bell size={15} />
          </button>
          {notifCount > 0 && (
            <span
              ...
              style={{ top: -3, right: -3, background: "var(--red)", color: "#fff", fontSize: "var(--fs-3xs)", borderRadius: "var(--r-pill)", minWidth: 16, height: 16, padding: "0 2px", border: "2px solid var(--bg-header)" }}
            >
              {notifCount}
            </span>
          )}
          ...
          {notifOpen && (
            ...
            >
              ...
              <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: "1px solid var(--border)" }}>
                <div className="text-white font-semibold" style={{ fontSize: "var(--fs-md)" }}>Notifications</div>
                <div className="flex items-center gap-2">
                  {notifCount > 0 && (
                    <span style={{ fontSize: "var(--fs-3xs)", background: "rgba(239,68,68,0.15)", color: "var(--danger-text)", borderRadius: "var(--r-pill)", padding: "2px 7px", fontWeight: 600 }}>
                      {notifCount} new
                    </span>
                  )}
              ...
              <div style={{ maxHeight: 360, overflowY: "auto" }}>
                {pendingVerifs.length === 0 && openDisputes.length === 0 && (
                  <div className="text-center py-8" style={{ fontSize: "var(--fs-sm)", color: "var(--text-muted)" }}>All caught up!</div>
                )}
                  ...
                  <div>
                    ...
                    {pendingVerifs.map((v) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-white font-medium" style={{ fontSize: "var(--fs-xs)" }}>{v.name} needs verification</div>
                          <div style={{ fontSize: "var(--fs-3xs)", color: "var(--text-muted)", marginTop: 2 }}>{v.documents.length} document{v.documents.length === 1 ? "" : "s"} · Submitted {v.date}</div>
                        </div>
                      </Link>
                    ))}
                  </div>
                  ...
                  <div>
                    ...
                    {openDisputes.map((d) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-white font-medium" style={{ fontSize: "var(--fs-xs)" }}>{d.jobTitle} — Dispute raised</div>
                          <div style={{ fontSize: "var(--fs-3xs)", color: "var(--text-muted)", marginTop: 2 }}>{d.clientName} · {d.amount} · {d.createdAt}</div>
                        </div>
                      </Link>
                    ))}
                  </div>
              ...
              <div style={{ borderTop: "1px solid var(--border)", padding: "10px 16px" }}>
                <Link
                  ...
                  style={{ fontSize: "var(--fs-xs)", color: "var(--indigo-light)", textDecoration: "none" }}
                >
